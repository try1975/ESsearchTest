...
namespace Gma.CodeCloud.Controls.TextAnalyses.Stemmers.Ru
{
    public class PorterRu : IWordStemmer
    {
        ...
        public static string TransformingWord(string word)
        {
            ...
            {
                var matches = Reflexive.Matches(rv);
                rv = ReplaceFirst(matches, rv);
                temp = Adjective.Matches(rv);
                stringTemp = ReplaceFirst(temp, rv);
                if (!stringTemp.Equals(rv))
                {
                    rv = stringTemp;
                    matches = Participle.Matches(rv);
                    rv = ReplaceFirst(matches, rv);
                }
                else
                {
